const PropertiesList = [
  {
    id: 1,
    title: `Casa Minimalista III`,
    location: `Altos de la Tahona, Canelones`,
    price: 284000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`data:image/jpeg;base64,`,`https://images.adsttc.com/media/images/622b/9c0b/6a2b/af01/6506/d38b/newsletter/004-ok.jpg`,`https://images.adsttc.com/media/images/61e9/8a5c/3522/8a01/670d/25b3/newsletter/casa-do-mar-mayresse-arquitetura-arch-image-claudio-fonseca-14.jpg`],
    details: {
      rooms: 3,
      toilets: 2,
      squareMeters: 1800,
      swimmingPool: true,
      garage: 2,
    },
  },
  {
    id: 2,
    title: `Monoambiente a 2 de la rambla`,
    location: `Centro, Montevideo`,
    price: 83000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://www.inmobiliariaya.com/fotos/1//106/24614/812251403339393.jpg`],
    details: {
      rooms: 1,
      toilets: 1,
      squareMeters: 32,
    },
  },
  {
    id: 3,
    title: `Moderna propiedad, 2 cuartos y 3 baños`,
    location: `Punta Gorda, Montevideo`,
    price: 398000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRedbxZc_yt3XHt6kuBUjvg5DoDVqY_e8hxCA&usqp=CAU`],
    details: {
      rooms: 2,
      toilets: 2,
      squareMeters: 214,
      garage: 1,
    },
  },
  {
    id: 4,
    title: `Casa en balneario del este`,
    location: `Punta del Este, Maldonado`,
    price: 276000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://static.tokkobroker.com/pictures/17889575032744678417131075648989918110151520165430534322217180322869061533444.jpg`],
    details: {
      rooms: 2,
      toilets: 2,
      squareMeters: 192,
      swimmingPool: true,
      garage: 1,
    },
  },
  {
    id: 5,
    title: `Casa en Carmel`,
    location: `Carmel, Canelones`,
    price: 260000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_995428-MLU52067832698_102022-W.jpg`],
    details: {
      rooms: 3,
      toilets: 2,
      squareMeters: 376,
      garage: 1,
    },
  },
  {
    id: 6,
    title: `Estrene En Solymar Norte`,
    location: `Solymar Norte, Canelones`,
    price: 125000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_758740-MLU52104923938_102022-O.webp`],
    details: {
      rooms: 2,
      toilets: 1,
      squareMeters: 200,
    },
  },
  {
    id: 7,
    title: `Excelente Construcción! En Doble Nivel 4 Dormitorios`,
    location: `Solymar, Canelones`,
    price: 285000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_753647-MLU52168945287_102022-O.webp`],
    details: {
      rooms: 4,
      toilets: 2,
      squareMeters: 32,
    },
  },
  {
    id: 8,
    title: `Casa 4 Dormitorios 3 Baños Piscina Solymar Sur`,
    location: `Solymar, Canelones`,
    price: 420000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_963917-MLU49876952301_052022-O.webp`],
    details: {
      rooms: 4,
      toilets: 3,
      squareMeters: 229,
      garage: 1,
      swimmingPool: true,
    },
  },
  {
    id: 9,
    title: `Apartamento En Peninsula`,
    location: `Punta del Este, Maldonado`,
    price: 85000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_944125-MLU51740899364_092022-O.webp`],
    details: {
      rooms: 1,
      toilets: 1,
      squareMeters: 25,
    },
  },
  {
    id: 10,
    title: `Apartamento De 2 Dormitorios Con Cochera`,
    location: `Punta Gorda, Montevideo`,
    price: 45490,
    availableFor: ["rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_782452-MLU52125838972_102022-O.webp`],
    details: {
      rooms: 2,
      toilets: 2,
      squareMeters: 90,
      garage: 1,
    },
  },
  {
    id: 11,
    title: `Apartamento En Venta Playa Mansa`,
    location: `Punta del Este, Maldonado`,
    price: 520000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_761398-MLU52163171868_102022-O.webp`],
    details: {
      rooms: 3,
      toilets: 3,
      squareMeters: 150,
    },
  },
  {
    id: 12,
    title: `Ph A Estrenar En Ciudad De La Costa`,
    location: `Solymar, Canelones`,
    price: 148000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_684530-MLU51371837314_092022-O.webp`],
    details: {
      rooms: 2,
      toilets: 1,
      squareMeters: 100,
    },
  },
  {
    id: 13,
    title: `Casa - El Pinar`,
    location: `El Pinar, Canelones`,
    price: 265000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_926109-MLU52196147650_102022-O.webp`],
    details: {
      rooms: 3,
      toilets: 2,
      squareMeters: 750,
      garage: 1,
      swimmingPool: true,
    },
  },
  {
    id: 14,
    title: `Espléndida Casa Sobre Lago, Con Excepcional Vista`,
    location: `Shangrilá, Canelones`,
    price: 550000,
    availableFor: ["sale", "rent"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_949722-MLU52220896886_102022-O.webp`],
    details: {
      rooms: 3,
      toilets: 4,
      squareMeters: 1120,
      swimmingPool: true,
      garage: 1
    },
  },
  {
    id: 15,
    title: `Apartamento De 2 Dormitorios Con Cochera`,
    location: `Lagomar, Canelones`,
    price: 265000,
    availableFor: ["sale"],
    imgUrl:  [`https://http2.mlstatic.com/D_NQ_NP_954759-MLU51045719546_082022-O.webp`],
    details: {
      rooms: 3,
      toilets: 3,
      squareMeters: 300,
      swimmingPool: true,
    },
  },
];

export default PropertiesList